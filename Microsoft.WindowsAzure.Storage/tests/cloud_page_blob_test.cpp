...
SUITE(Blob)
{
    ...
    TEST_FIXTURE(page_blob_test_base, page_blob_prevsnapshot_url)
    {
        ...
        auto diff = m_blob.download_page_ranges_diff(get_snapshot_url(snapshot1), azure::storage::access_condition(), azure::storage::blob_request_options(), m_context);
        CHECK(0 == diff.size());
...

        {
            ...
            m_blob.upload_pages(stream, 0, _XPLATSTR(""));
            m_blob.clear_pages(512, 512);
            diff = m_blob.download_page_ranges_diff(get_snapshot_url(snapshot2), azure::storage::access_condition(), azure::storage::blob_request_options(), m_context);
            CHECK(2 == diff.size());
            if (diff[0].is_cleared_rage() == true)
            {
                auto temp = diff[0];
        ...
        azure::storage::cloud_page_blob snapshot3 = m_blob.create_snapshot(azure::storage::cloud_metadata(), azure::storage::access_condition(), azure::storage::blob_request_options(), m_context);
        auto diff3 = snapshot3.download_page_ranges_diff(get_snapshot_url(snapshot2), azure::storage::access_condition(), azure::storage::blob_request_options(), m_context);
        CHECK(2 == diff.size());
        if (diff[0].is_cleared_rage() == true)
        {
            auto temp = diff[0];
...

        {
            ...
            auto stream = concurrency::streams::bytestream::open_istream(std::move(utf8_body));
            m_blob.upload_pages(stream, 0, _XPLATSTR(""));
            diff = m_blob.download_page_ranges_diff(get_snapshot_url(snapshot1), azure::storage::access_condition(), azure::storage::blob_request_options(), m_context);
            CHECK(1 == diff.size());
            CHECK_EQUAL(false, diff[0].is_cleared_rage());
            CHECK(0 == diff[0].start_offset());
            CHECK(2047 == diff[0].end_offset());
